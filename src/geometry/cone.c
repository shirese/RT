...
static double			cone_beta(double *expr, t_geo *geo, t_ray *r)
{
	...
	c = (t_cone *)geo->curr;
	res = (2 * alpha_cone(expr[0], c->axis.x, r->dir.x) *\
		beta_cone(expr[1], c->axis.x, r->origin.x, geo->origin->x)) +\
		(2 * alpha_cone(expr[0], c->axis.y, r->dir.y) *\
		beta_cone(expr[1], c->axis.y, r->origin.y, geo->origin->y)) + \
		(2 * alpha_cone(expr[0], c->axis.z, r->dir.z) *\
...
static double			cone_alpha(double expr, t_cone *c, t_ray *r)
{
	...
	res = pow(alpha_cone(expr, c->axis.x, r->dir.x), 2) + \
	pow(alpha_cone(expr, c->axis.y, r->dir.y), 2) + \
	pow(alpha_cone(expr, c->axis.z, r->dir.z), 2) - \
...
void					cone_solutions(t_geo *geo, t_ray *r, t_hp *sol)
{
	...
	expr[0] = c->axis.x * r->dir.x + c->axis.y * r->dir.y + \
		c->axis.z * r->dir.z;
	...
	if (abcd[3] >= 0 && (abcd[3] = sqrt(abcd[3])) > 0)
	{
		sol[0].t = positive_smallest((-abcd[1] - abcd[3]) / \
			(2 * abcd[0]), (-abcd[1] + abcd[3]) / (2 * abcd[0]));
		sol[1].t = non_positive_smallest((-abcd[1] - abcd[3]) / \
			(2 * abcd[0]), (-abcd[1] + abcd[3]) / (2 * abcd[0]));
